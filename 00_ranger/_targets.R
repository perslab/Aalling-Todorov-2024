...
source("../00_cellbender/map_ref.R")
...
stage_01 = list(
  tar_target(path_to_cb, 
             paste0('../data/cellbender/raw_h5'), 
             format = "file",
             packages=c("tidyverse", "Seurat", "scCustomize")),
  tar_target(path_to_cb_rds, 
             paste0('../data/cellbender/cellbender_h5'), 
             format = "file",
             packages=c("tidyverse", "Seurat", "scCustomize")),
  tar_target(path_to_sample_meta, 
             paste0('../data/meta/metadata.csv'), 
             format = "file",
             packages=c("tidyverse", "Seurat")),
  tar_target(obj_cb_00,
             path_to_cb %>%
             make_mat %>%
             fix_mat_colnames_ranger %>%
             make_obj_from_mat),
  tar_target(cb_metadata,
             make_metadata(obj_cb_00, path_to_cb_rds, path_to_sample_meta)),
...
stage_02 = list(
    tar_target(path_to_class_markers, 
               paste0('../00_cellbender/marker_major_class.txt'), 
               format = "file"),
    ...
    tar_target(path_to_other_markers, 
               paste0('../00_cellbender/non_neuron_markers.txt'), 
               format = "file"),
    ...
    tar_target(obj_cb_other_00,
               ...
               process_seurat(method = "integrate", batch ="Index.10x_SCOP", dims = 30, res = 0.8),
               packages=c("tidyverse", "Seurat", "CellAnnotatoR")),
    tar_target(label_chunks_other_path, paste0('../00_cellbender/cluster_surgery/labels_chunk_other.qs'), format = "file"),
...
# neuron cluster surgery
    tar_target(label_chunks_neuron_path, paste0("../", '00_cellbender/cluster_surgery/labels_chunk_neuron_v01.qs'), format = "file"),
   ...
   tar_target(obj_cb_neuron_00,
    ...
    tar_target(path_to_tany_markers, 
               paste0('../00_cellbender/tany_markers.txt'), 
               format = "file"),    
